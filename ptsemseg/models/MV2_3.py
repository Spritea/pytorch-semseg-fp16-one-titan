...
class RefineBlock(nn.Module):
    def __init__(self, in_channel,kernal_size):
        ...
        if kernal_size==3:
            ...
            self.c3_1=nn.Sequential(nn.Conv2d(512,512,kernel_size=(1, kernal_size),padding=(0,pad),bias=False),
                                nn.Conv2d(512, 512, kernel_size=(kernal_size,1), padding=(pad,0), bias=False))
        self.bn = nn.BatchNorm2d(512)
        self.relu = nn.ReLU(inplace=True)
        if kernal_size==3:
            self.c3_2 = nn.Conv2d(512, 512, kernel_size=kernal_size, stride=1, padding=(kernal_size-1)/2, bias=False)
        else:
            self.c3_2 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=(1, kernal_size), padding=(0,pad), bias=False),
                                  nn.Conv2d(512, 512, kernel_size=(kernal_size, 1), padding=(pad,0), bias=False))
...
class MV2_3_ResNet(nn.Module):

    def __init__(self, block, layers, num_classes=1000):
        super(MV2_3_ResNet, self).__init__()
        self.inplanes = 64
...
def MV2_3_ResNet18(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_3_ResNet(BasicBlock, [2, 2, 2, 2], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '18_imagenet'
        url = models_urls[key]
...
def MV2_3_ResNet34(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_3_ResNet(BasicBlock, [3, 4, 6, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '34_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_3_ResNet50(num_classes, pretrained=True, **kwargs):
    """Constructs a MV1_ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_3_ResNet(Bottleneck, [3, 4, 6, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '50_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_3_ResNet101(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_3_ResNet(Bottleneck, [3, 4, 23, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '101_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_3_ResNet152(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-152 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_3_ResNet(Bottleneck, [3, 8, 36, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '152_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
