...
class FPA_dense(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(FPA_dense, self).__init__()

        self.c7_1 = nn.Conv2d(in_channel*3, out_channel, kernel_size=7, stride=1, padding=3, bias=False)
        self.c5_1 = nn.Conv2d(in_channel*2, out_channel, kernel_size=5, stride=1, padding=2, bias=False)
    ...
    def forward(self, x):
        ...
        x5_1_input=torch.cat([x,x3_2],dim=1)

        x5_1 = self.c5_1(x5_1_input)
        x5_1 = self.bn(x5_1)
        x5_1 = self.relu(x5_1)
        x5_2 = self.c5_2(x5_1)
        x5_2 = self.bn(x5_2)

        x7_1_input=torch.cat([x,x3_2,x5_2],dim=1)

        x7_1 = self.c7_1(x7_1_input)
        x7_1 = self.bn(x7_1)
        x7_1 = self.relu(x7_1)
        x7_2 = self.c7_2(x7_1)
        x7_2 = self.bn(x7_2)
...
# MV2_1 add dense FPA, similar to DenseAspp
class MV2_8_ResNet(nn.Module):

    def __init__(self, block, layers, num_classes=1000):
        super(MV2_8_ResNet, self).__init__()
        self.inplanes = 64
...
def MV2_8_ResNet18(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_8_ResNet(BasicBlock, [2, 2, 2, 2], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '18_imagenet'
        url = models_urls[key]
...
def MV2_8_ResNet34(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_8_ResNet(BasicBlock, [3, 4, 6, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '34_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_8_ResNet50(num_classes, pretrained=True, **kwargs):
    """Constructs a MV1_ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_8_ResNet(Bottleneck, [3, 4, 6, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '50_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_8_ResNet101(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_8_ResNet(Bottleneck, [3, 4, 23, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '101_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
...
def MV2_8_ResNet152(num_classes, pretrained=False, **kwargs):
    """Constructs a MV1_ResNet-152 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MV2_8_ResNet(Bottleneck, [3, 8, 36, 3], **kwargs, num_classes=num_classes)
    if pretrained:
        key = '152_imagenet'
        url = models_urls[key]
        model.load_state_dict(maybe_download(key, url), strict=False)
